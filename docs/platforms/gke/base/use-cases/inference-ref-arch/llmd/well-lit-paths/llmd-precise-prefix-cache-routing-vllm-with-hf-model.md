...
[llm-d precise prefix cache routing well-lit path](https://github.com/llm-d/llm-d/tree/main/guides/precise-prefix-cache-routing).
In this guide, you will see how you can install llmd precise prefix cache
routing well-lit path on GKE using our
[GKE Inference reference implementation](/platforms/gke/base/use-cases/inference-ref-arch/terraform/README.md).
The
[well-lit](https://github.com/llm-d/llm-d/tree/main/guides/precise-prefix-cache-routing)
path provides you the instructions to create the precise prefix cache routing
...
and apply llm-d optimization-baseline layer on top of it.
...
The `deploy-llmd-precise-prefix-cache-routing.sh` script will perform the
following steps:

- Set up base GKE cluster platform and other infrastructure resources required
  for llm-d.
- Create Gateway and Router for llm-d precise prefix cache routing. You will
  create model server manually in the following sections of this guide.
...
- Configure the model server
  ...
  - If you are running it on GPU
    ...
    ```shell
    "${ACP_REPO_DIR}/platforms/gke/base/use-cases/inference-ref-arch/kubernetes-manifests/online-inference-gpu/llmd-precise-prefix-cache-routing/vllm/configure_vllm.sh"
    ```

  - if you are running it on TPU

    ```shell
    "${ACP_REPO_DIR}/platforms/gke/base/use-cases/inference-ref-arch/kubernetes-manifests/online-inference-tpu/llmd-precise-prefix-cache-routing/vllm/configure_vllm.sh"
    ```
...
- Deploy the model server
  ...
  - If you are running it on GPU
    ...
    ```shell
    kubectl apply --kustomize "${ACP_REPO_DIR}/platforms/gke/base/use-cases/inference-ref-arch/kubernetes-manifests/online-inference-gpu/llmd-precise-prefix-cache-routing/vllm/${ACCELERATOR_TYPE}-${HF_MODEL_NAME}"
    ```

  - If you are running it on TPU

    ```shell
    kubectl apply --kustomize "${ACP_REPO_DIR}/platforms/gke/base/use-cases/inference-ref-arch/kubernetes-manifests/online-inference-tpu/llmd-precise-prefix-cache-routing/vllm/${ACCELERATOR_TYPE}-${HF_MODEL_NAME}"
    ```
...
- Watch the deployment until it is ready.
  ...
  - If you are running it on GPU
    ...
    ```shell
    watch --color --interval 5 --no-title \
    "kubectl --namespace=${llmd_namespace} get deployment/precise-prefix-cache-routing-gpu-vllm-decode-${ACCELERATOR_TYPE}-${HF_MODEL_NAME} | GREP_COLORS='mt=01;92' egrep --color=always -e '^' -e '1/1     1            1'
    echo '\nLogs(last 10 lines):'
    kubectl --namespace=${llmd_namespace} logs deployment/precise-prefix-cache-routing-gpu-vllm-decode-${ACCELERATOR_TYPE}-${HF_MODEL_NAME} --all-containers --tail 10"
    ```
...
- Check the all the deployments
  ...
  precise-prefix-cache-routing-epp                            1/1     1            1           XX
  precise-prefix-cache-routing-nvidia-gpu-vllm-decode-XXX     2/2     1            1           XX
  ```

  Note:

  - precise-prefix-cache-routing-epp is the Gateway API Inference Extension
    endpoint picker.
  - precise-prefix-cache-routing-nvidia-gpu-vllm-decode-XXX is the model server
    running inference of the model you chose. It may take some time for this
    deployment to be up completely depending upon the GPU availability
...
- Check all the resources
  ...
  pod/precise-prefix-cache-routing-epp-XXXX                  2/2     Running   0          XX
  pod/precise-prefix-cache-routing-epp-XXXX                  2/2     Running   0          XX
  pod/precise-prefix-cache-routing-gpu-vllm-decode-XXXX      1/1     Running   0          XX
  pod/precise-prefix-cache-routing-gpu-vllm-decode-XXXX      1/1     Running   0          XX

  NAME                                                TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)                    AGE
  service/precise-prefix-cache-routing-epp            ClusterIP   XXXX            <none>        9002/TCP,9090/TCP,80/TCP   XX
  service/precise-prefix-cache-routing-ips-XXXX       ClusterIP   None            <none>        54321/TCP                  XX

  NAME                                                                   READY   UP-TO-DATE   AVAILABLE   AGE
  deployment.apps/precise-prefix-cache-routing-epp                       2/2     2            2           XX
  deployment.apps/precise-prefix-cache-routing-gpu-vllm-decode-XXXX      2/2     2            2           XX

  NAME                                                                   DESIRED   CURRENT   READY   AGE
  replicaset.apps/precise-prefix-cache-routing-epp-XXXX                  2         2         2       XX
...
- Wait for the model server deployment to be ready before sending the request to
  ...
  - If you are running it on GPU:
    ...
    ```
    watch --color --interval 5 --no-title   "kubectl --namespace=${llmd_namespace} get deployment.apps/precise-prefix-cache-routing-gpu-vllm-decode-${ACCELERATOR_TYPE}-${HF_MODEL_NAME} | GREP_COLORS='mt=01;92' egrep --color=always -e '^' -e '1/1     1            1'"
    ```
...
- When the deployment is ready, you will output similar to the following
  ...
  ```
  NAME                                                READY   UP-TO-DATE   AVAILABLE   AGE
  precise-prefix-cache-routing-gpu-vllm-decode-XXXX   2/2     2            2           XX
  ```
