...
class GeminiNode25:
    def __init__(
        ...
        try:
            ...
            if not self.region:
                # GCP metadata for zone is like 'projects/12345/zones/us-central1-a'
                zone_metadata = get_gcp_metadata("instance/zone")
                if zone_metadata:
                    zone = zone_metadata.split("/")[-1]
                    self.region = "-".join(zone.split("-")[:-1])
                else:
                    print("Could not retrieve GCP zone metadata.")

        except Exception as e:
            print(f"Failed to get GCP metadata (this is expected if not on GCP): {e}")
            # Don't raise here, allow project/region to be provided or fail validation
        ...
        try:
            ...
            print(f"Failed to initialize genai.Client for Vertex AI: {e}")
