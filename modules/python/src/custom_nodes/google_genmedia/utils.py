...
def download_gcsuri(gcsuri: str, destination: str) -> bool:
    ...
    Raises:
        ...
        google.api_core.exceptions.NotFound: If the GCS bucket or object does not exist.
        google.api_core.exceptions.Forbidden: If there are insufficient permissions to access the GCS object.
        exceptions.APICallError: If the download fails after all retries due to a persistent service error.
    """
...
def generate_image_from_text(
    ...
    Raises:
        exceptions.ConfigurationError: If the API returns a non-retryable client error
                                       (e.g., invalid API key, permissions error).
        exceptions.APICallError: If the API call fails after all retries, if the image generation
                                 is blocked by safety filters, or if the response is empty.
    """
...
def generate_video_from_gcsuri_image(
    ...
    Raises:
        exceptions.ConfigurationError: If input parameters are invalid (e.g., using 'lossless'
                                       quality without an `output_gcs_uri`).
        exceptions.APICallError: If the API call fails, either initially after retries
                                 or during the status polling loop.
        RuntimeError: If video generation succeeds but processing or saving the
                      resulting video files fails.
    """
...
def generate_video_from_image(
    ...
    Raises:
        exceptions.ConfigurationError: If input parameters are invalid (e.g., unsupported image format,
                                       or 'lossless' quality without `output_gcs_uri`).
        exceptions.FileProcessingError: If the input image tensor fails to be converted to bytes.
        exceptions.APICallError: If the API call fails, either initially after retries
                                 or during the status polling loop.
        RuntimeError: If video generation succeeds but processing or saving the
                      resulting video files fails.
...
def validate_gcs_uri_and_image(
    ...
    Raises:
        google.api_core.exceptions.Forbidden: If there are insufficient permissions to access the GCS bucket or object.
        exceptions.APICallError: If a GCS API call fails after all retries.
    """
