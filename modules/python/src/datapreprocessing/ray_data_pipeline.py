...
class SingleFilenameProvider(FilenameProvider):
    ...
    def get_filename_for_block(self, block, task_index, block_index, shard_spec=None, *args, **kwargs) -> str:
...
class RayDataPipelineOrchestrator:
    ...
    def execute(self, input_bucket: str, input_path: str, output_bucket: str, output_path: str, output_image_folder: str):
        if self.ray_cluster_host and self.ray_cluster_host != "local":
            ...
            if not target_address.startswith("ray://") and not target_address.startswith("http://"):
            ...
            if not ray.is_initialized():
                logger.info("Running pipeline via localized cluster/driver tracking context...")
                ray.init(runtime_env=self.ray_runtime_env)

        logger.info("Ray streaming connection active. Initializing zero-copy GCS lazy read...")
        ...
        logger.info(f"Targeting consolidated base output folder path: {output_directory_uri}")
        ...
        required_cols = ["uniq_id", "product_name", "description", "brand", "image", "product_specifications", "product_category_tree"]
        filter_null_cols = ["description", "image", "product_specifications", "product_category_tree"]
        ...
        logger.info(f"Coalescing data blocks down to single partition file: {target_filename}")
