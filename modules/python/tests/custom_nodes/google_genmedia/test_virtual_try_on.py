...
class TestVirtualTryOn(unittest.TestCase):
    ...
    @patch('src.custom_nodes.google_genmedia.virtual_try_on.VirtualTryOn.__init__', lambda *args, **kwargs: None)
    @patch('src.custom_nodes.google_genmedia.virtual_try_on.utils.tensor_to_pil_to_base64')
    @patch('src.custom_nodes.google_genmedia.virtual_try_on.utils.base64_to_pil_to_tensor')
    def test_generate_and_return_image_success(self, mock_base64_to_tensor, mock_tensor_to_base64):
        # Arrange
        person_image = torch.rand(1, 512, 512, 3)
        product_image = torch.rand(1, 512, 512, 3)
        
        mock_tensor_to_base64.side_effect = ["person_base64_string", "product_base64_string"]
