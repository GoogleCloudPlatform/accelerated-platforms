...
BASH_SOURCE_CWS_MY_PATH="$(
  cd "$(dirname "${BASH_SOURCE}")" >/dev/null 2>&1
  pwd -P
)"

ACP_REPO_DIR=$(realpath "${BASH_SOURCE_CWS_MY_PATH}/../../../../")
ACP_PLATFORM_DIR="${ACP_REPO_DIR}/platforms/cws"

if [[ ! -v CWS_SHARED_CONFIG_PATHS ]]; then
  if [ "$#" -eq 0 ]; then
    CWS_SHARED_CONFIG_PATHS=("${ACP_PLATFORM_DIR}/_shared_config")
  else
    CWS_SHARED_CONFIG_PATHS=("${@}")
  fi
fi

if ! grep -q "platform_default_project_id" "${ACP_PLATFORM_DIR}/_shared_config/platform.auto.tfvars"; then
...
for SHARED_CONFIG_PATH in "${CWS_SHARED_CONFIG_PATHS[@]}"; do
