...
locals {
  cluster_credentials_command_gke  = "gcloud container clusters get-credentials ${local.workstation_cluster_name} --dns-endpoint --location ${var.workstation_cluster_region} --project ${local.workstation_cluster_project_id}"
  cluster_credentials_command_gkee = "gcloud container fleet memberships get-credentials ${local.workstation_cluster_name} --project ${local.workstation_cluster_project_id}"
  ...
  cluster_gcsfuse_user_role        = "projects/${local.workstation_cluster_project_id}/roles/${local.cluster_gcsfuse_user_role_name}"
  ...
  cluster_gcsfuse_viewer_role      = "projects/${local.workstation_cluster_project_id}/roles/${local.cluster_gcsfuse_viewer_role_name}"
  ...
  workstation_cluster_name = local.unique_identifier_prefix

  cluster_node_pool_service_account_id         = var.cluster_node_pool_default_service_account_id != null ? var.cluster_node_pool_default_service_account_id : "vm-${local.workstation_cluster_name}"
  cluster_node_pool_service_account_project_id = var.cluster_node_pool_default_service_account_project_id != null ? var.cluster_node_pool_default_service_account_project_id : local.workstation_cluster_project_id
...
variable "workstation_cluster_region" {
  ...
  validation {
    ...
    var.workstation_cluster_region)
    error_message = "'workstation_cluster_region' must be one of ['us-central1', 'us-east4', 'us-east5']"
  }
}
