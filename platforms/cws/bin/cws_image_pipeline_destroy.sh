...
ACP_PLATFORM_STACK_DIR="${ACP_PLATFORM_DIR}/cluster"

# shellcheck source=/dev/null
source "${ACP_PLATFORM_STACK_DIR}/cluster/_shared_config/scripts/set_environment_variables.sh"
...
for terraservice in "${terraservices[@]}"; do
  cd "${ACP_PLATFORM_STACK_DIR}/terraform/${terraservice}" &&
    ...
    rm -f bucket.tf
    gcloud storage rm -r "gs://${terraform_bucket_name:?}/terraform/cws/cluster/${terraservice}"
    rm -rf .terraform/
done
