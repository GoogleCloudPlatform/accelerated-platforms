...
resource "google_cloudbuildv2_repository" "gc_cloud_workstation_image" {
  ...
  project           = data.google_project.cloudbuild.project_id
  remote_uri        = "https://${var.cloudbuild_cws_image_pipeline_git_provider}/${var.cloudbuild_cws_image_pipeline_git_namespace}/${var.cloudbuild_cws_image_pipeline_git_repository_name}.git"
}
