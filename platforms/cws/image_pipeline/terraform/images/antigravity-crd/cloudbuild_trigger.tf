...
resource "google_cloudbuild_trigger" "cws_image_antigravity_crd" {
  filename = "cloudbuild/antigravity-crd.yaml"
  included_files = [
    "cloudbuild/antigravity-crd.yaml",
    "container-images/antigravity-crd/**",
  ]
  location        = local.cloudbuild_location
  name            = "${local.unique_identifier_prefix}-cws-image-antigravity-crd"
