...
locals {
  ...
  local_directory      = "platforms/cws/image_pipeline/terraform/images/code-oss/repo_files"
}

resource "local_file" "cloudbuild_yaml" {
  content = templatefile(
    "${path.module}/templates/repository/cloudbuild/code-oss.yaml",
    {
      cloudbuild_cws_image_registry_name = local.cloudbuild_cws_image_registry_name,
    }
  )
  file_permission = "0644"
  filename        = "${local.acp_root}/${local.local_directory}/cloudbuild/code-oss.yaml"
}

resource "local_file" "dockerfile" {
  content = templatefile(
    "${path.module}/templates/repository/container-images/code-oss/Dockerfile",
    {
      cloudbuild_cws_image_registry_upstream_name = local.cloudbuild_cws_image_registry_upstream_name,
    }
  )
  file_permission = "0644"
  filename        = "${local.acp_root}/${local.local_directory}/container-images/code-oss/Dockerfile"
}
