...
resource "local_file" "cloudbuild_yaml" {
  content = templatefile(
    "${path.module}/templates/repository/cloudbuild/comfyui-cpu.yaml",
    {
...
resource "local_file" "dockerfile" {
  content = templatefile(
    "${path.module}/templates/repository/container-images/comfyui/cpu/Dockerfile",
    {
      cloudbuild_cws_image_registry_upstream_name = local.cloudbuild_cws_image_registry_upstream_name,
    }
  )
  file_permission = "0644"
  filename        = "${local.acp_root}/${local.local_directory}/container-images/comfyui/cpu/Dockerfile"
}

resource "local_file" "start_ide" {
  content         = file("${path.module}/templates/repository/container-images/comfyui/cpu/etc/workstation-startup.d/110_start-comfyui.sh")
  file_permission = "0644"
  filename        = "${local.acp_root}/${local.local_directory}/container-images/comfyui/cpu/etc/workstation-startup.d/110_start-comfyui.sh"
}
