...
resource "google_cloud_scheduler_job" "cws_image_daily" {
  name      = "${local.unique_identifier_prefix_underscore}-cws-image-comfyui-nvidia-schedule"
  ...
  http_target {
    ...
    http_method = "POST"
    uri         = "${local.cloudbuild_trigger_url_prefix}/${google_cloudbuild_trigger.cws_image.trigger_id}:run"
