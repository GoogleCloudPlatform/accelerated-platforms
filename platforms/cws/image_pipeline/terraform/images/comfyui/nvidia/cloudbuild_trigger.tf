...
resource "google_cloudbuild_trigger" "cws_image" {
  filename = "cloudbuild/comfyui-nvidia.yaml"
  included_files = [
    "cloudbuild/comfyui-nvidia.yaml",
    "container-images/comfyui/nvidia/**",
  ]
  location        = local.cloudbuild_location
  name            = "${local.unique_identifier_prefix}-cws-image-comfyui-nvidia"
