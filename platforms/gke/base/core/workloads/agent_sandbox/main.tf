...
locals {
  ...
  manifests_directory         = "${local.namespace_directory}/agent-sandbox-system"
  namespace_directory         = "${local.manifests_directory_root}/namespace"
  version_manifests_directory = "${path.module}/manifests/agent-sandbox-${var.agent_sandbox_version}"
}
...
cp -r templates/namespace-agent-sandbox-system.yaml "${self.input.manifests_dir}/"
EOT
    interpreter = ["bash", "-c"]
...
module "kubectl_apply_namespace" {
  ...
  manifest                    = "${local.namespace_directory}/namespace-agent-sandbox-system.yaml"
