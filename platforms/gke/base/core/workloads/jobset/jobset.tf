...
resource "null_resource" "jobset_manifests" {
  provisioner "local-exec" {
    command     = <<EOT
mkdir -p ${self.triggers.manifests_dir}
wget https://github.com/kubernetes-sigs/jobset/releases/download/v${self.triggers.version}/manifests.yaml -O ${self.triggers.manifests_dir}/manifests.yaml
...
EOT
  ...
  triggers = {
    ...
    manifests_dir = "${local.manifests_directory}/jobset-${var.jobset_version}"
    version       = var.jobset_version
  }
}

resource "null_resource" "jobset_manifests_apply" {
  depends_on = [
    null_resource.cluster_credentials,
    null_resource.jobset_manifests,
  ]
