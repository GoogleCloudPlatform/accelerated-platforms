...
resource "null_resource" "lws_manifests" {
  provisioner "local-exec" {
    command     = <<EOT
mkdir -p ${self.triggers.manifests_dir}
wget https://github.com/kubernetes-sigs/lws/releases/download/v${self.triggers.version}/manifests.yaml -O ${self.triggers.manifests_dir}/manifests.yaml
...
EOT
  ...
  triggers = {
    ...
    manifests_dir = "${local.manifests_directory}/lws-${var.lws_version}"
    version       = var.lws_version
  }
}

resource "null_resource" "lws_manifests_apply" {
  depends_on = [
    null_resource.cluster_credentials,
    null_resource.lws_manifests,
  ]
