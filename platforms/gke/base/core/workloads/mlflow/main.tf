...
locals {
  ...
  manifests_directory         = "${local.namespace_directory}/mlflow-system"
  namespace_directory         = "${local.manifests_directory_root}/namespace"
  version_manifests_directory = "${path.module}/manifests/mlflow-${var.mlflow_version}"
}
...
resource "terraform_data" "namespace" {
  input = {
    ...
    namespace     = local.mlflow_kubernetes_namespace
  }

  provisioner "local-exec" {
    command     = <<EOT
mkdir -p "${self.input.manifests_dir}" && \
cp -r templates/namespace-mlflow-system.yaml "${self.input.manifests_dir}/"
EOT
    interpreter = ["bash", "-c"]
...
module "kubectl_apply_namespace" {
  ...
  manifest                    = "${local.namespace_directory}/namespace-mlflow-system.yaml"
  manifest_includes_namespace = true
}

resource "local_file" "mlflow_manifests" {
  content = templatefile(
    "${path.module}/templates/workload/manifests.yaml",
    {
      bucket_name          = data.google_storage_bucket.mlflow.name,
      service_account_name = local.mlflow_kubernetes_service_account_name,
      mlflow_version       = var.mlflow_version
    }
  )
  filename = "${path.module}/templates/workload/manifests.yaml.rendered"
}

resource "terraform_data" "manifests" {
  depends_on = [
    local_file.mlflow_manifests,
  ]
...
mkdir -p "${self.input.manifests_dir}" && \
cp templates/workload/manifests.yaml.rendered "${self.input.version_manifests_dir}/manifests.yaml" && \
cp templates/workload/kustomization.yaml "${self.input.version_manifests_dir}/" && \
cp -r templates/workload/patch "${self.input.version_manifests_dir}/" && \
...
EOT
  ...
  triggers_replace = {
    ...
    version               = var.mlflow_version
    version_manifests_dir = local.version_manifests_directory
    manifest_rendered_md5 = local_file.mlflow_manifests.content_md5
  }
}
