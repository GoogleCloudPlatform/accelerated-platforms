...
resource "terraform_data" "cleanup_network_endpoint_groups" {
  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      negs=$(gcloud compute network-endpoint-groups list --filter="name~'k8s.*-${self.input.namespace}-.*' AND network~'.*-${self.input.identifier}$'" --format='value(format("{0},{1}", name, zone.basename()))' --project=${self.input.project_id})
      for neg in $${negs}; do
          name="$${neg%,*}"
          zone="$${neg#*,}"
