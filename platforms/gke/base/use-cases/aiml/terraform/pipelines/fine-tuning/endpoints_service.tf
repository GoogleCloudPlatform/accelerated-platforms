...
locals {
  ...
  endpoints = {
    gradio = {
      host         = "gradio.${var.fine_tuning_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 8080
      service_name = "gradio-svc"
    },
    locust = {
      host         = "locust.${var.fine_tuning_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 8089
      service_name = "locust-master-web-svc"
    },
    mlflow-tracking = {
      host         = "mlflow-tracking.${var.fine_tuning_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 5000
      service_name = "mlflow-tracking-svc"
    },
    ray-dashboard = {
      host         = "ray-dashboard.${var.fine_tuning_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 8265
