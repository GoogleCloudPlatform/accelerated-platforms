...
locals {
  gateway_manifests_directory = "${local.fine_tuning_manifests_directory}/namespace/${var.fine_tuning_namespace}/gateway"
...
resource "google_compute_managed_ssl_certificate" "external_gateway" {
  ...
  name    = "${local.unique_identifier_prefix}-${var.fine_tuning_namespace}-external-gateway"
...
resource "google_compute_global_address" "external_gateway_https" {
  name    = "${local.unique_identifier_prefix}-${var.fine_tuning_namespace}-external-gateway-https"
  project = data.google_project.cluster.project_id
}
...
resource "google_iap_client" "endpoint" {
  ...
  display_name = "IAP-gkegw-${local.unique_identifier_prefix}-${var.fine_tuning_namespace}-${each.key}"
}
...
###############################################################################
resource "null_resource" "gateway_manifest" {
  depends_on = [
    null_resource.namespace_manifest
  ]
  ...
  provisioner "local-exec" {
    command = "kubectl --namespace=${self.triggers.namespace} delete --filename=${self.triggers.manifests}; exit 0"
    environment = {
      KUBECONFIG = self.triggers.kubeconfig_file
    }
    interpreter = ["bash", "-c"]
  ...
  triggers = {
    ...
    manifests       = local_file.gateway_external_https_yaml.filename
    namespace       = var.fine_tuning_namespace
  }
}

resource "null_resource" "route_manifest" {
  depends_on = [
    null_resource.namespace_manifest
  ]
  ...
  triggers = {
    kubeconfig_file = data.local_file.kubeconfig.filename
    manifests       = local_file.route[each.key].filename
    namespace       = var.fine_tuning_namespace
  }
}

resource "null_resource" "gcp_backend_policy_manifest" {
  depends_on = [
    null_resource.namespace_manifest
  ]
  ...
  triggers = {
    kubeconfig_file = data.local_file.kubeconfig.filename
    manifests       = local_file.gcp_backend_policy[each.key].filename
    namespace       = var.fine_tuning_namespace
  }
}
