...
1.  Optionally, you can make the required environment variables configuration
    ...
    ```shell
    {
      export echo "ACP_REPO_DIR=${ACP_REPO_DIR}"
      export echo "ACP_PLATFORM_BASE_DIR=${ACP_PLATFORM_BASE_DIR}"
      export echo "ACP_PLATFORM_CORE_DIR=${ACP_PLATFORM_CORE_DIR}"
    } >>"${HOME}/.bashrc"
    ```
...
- GKE Enterprise features already enabled in the Google Cloud project:
  ...
  - [Import the `gke_hub_feature` resources](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/gke_hub_feature#import)
    ...
    Terraform state, we recommend that you the impact on other GKE clusters in
    the same project that depend on those resources. For example, when you
    destroy this reference architecture, these resources will be destroyed too,
    potentially impacting other GKE clusters in the same project.
