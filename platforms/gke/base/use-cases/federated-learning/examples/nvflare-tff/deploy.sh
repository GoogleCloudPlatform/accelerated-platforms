...
if [[ "${WORKLOAD_NAME}" == "server1" ]]; then
  ...
  check_optional_argument "${SERVER_IP_ADDRESS:-}" "${SERVER_IP_ADDRESS_DESCRIPTION}" || true
fi
...
for configuration_variable in "${NVFLARE_EXAMPLE_TERRAFORM_CONFIGURATION_VARIABLES[@]}"; do
  ...
  if [[ "${WORKLOAD_NAME}" == "server1" ]]; then
    ...
    if [[ "${configuration_variable}" =~ ^federated_learning_nvidia_flare_tff_example_server* ]] &&
      [[ -z "${SERVER_IP_ADDRESS:-}" ]]; then
      echo "Skip setting configuration variable for ${WORKLOAD_NAME} because it's applicable to clients but the server IP address is not set: ${configuration_variable}"
      continue
    fi
  fi
...
# Check if the variable to substitute to the placeholder has been set because it might be an optional parameter
if [[ "${WORKLOAD_NAME}" == "server1" ]]; then
  ...
  [[ -n "${SERVER_IP_ADDRESS:-""}" ]] && edit_terraform_configuration_variable_value_in_file "federated_learning_nvidia_flare_tff_example_server1_a_rrdatas_placeholder" "${SERVER_IP_ADDRESS}" "${FEDERATED_LEARNING_CONFIG_AUTO_VARS_FILE}"
fi
