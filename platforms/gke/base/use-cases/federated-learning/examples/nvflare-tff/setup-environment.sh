...
NVFLARE_EXAMPLE_TERRAFORM_INIT_CONFIGURATION_VARIABLES=(
  ...
  "federated_learning_cloud_storage_buckets = {\"nvf-ws\" = { force_destroy = true, versioning_enabled = false }}"
)

# shellcheck disable=SC2034 # Variable is used in other scripts
NVFLARE_EXAMPLE_TERRAFORM_CONFIGURATION_VARIABLES=(
  "federated_learning_cloud_storage_buckets_iam_bindings = [ {bucket_name = \"nvf-ws\", member = \"federated_learning_nvidia_flare_tff_apps_service_account_placeholder\", role = \"roles/storage.objectUser\"} ]"
