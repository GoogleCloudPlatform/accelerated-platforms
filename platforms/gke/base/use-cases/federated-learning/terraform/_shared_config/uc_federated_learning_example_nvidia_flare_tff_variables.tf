...
variable "federated_learning_nvidia_flare_tff_example_workload_to_deploy" {
  ...
  validation {
    condition     = var.federated_learning_nvidia_flare_tff_example_workload_to_deploy == "client1" || var.federated_learning_nvidia_flare_tff_example_workload_to_deploy == "client2" || var.federated_learning_nvidia_flare_tff_example_workload_to_deploy == "server1"
    error_message = "Valid values are: client1, client2, server1"
  }
}

variable "federated_learning_nvidia_flare_tff_example_client1_a_rrdatas" {
  default     = []
  description = "Value of the A record for client1"
  type        = list(string)
}

variable "federated_learning_nvidia_flare_tff_example_client2_a_rrdatas" {
  default     = []
  description = "Value of the A record for client2"
  type        = list(string)
}

variable "federated_learning_nvidia_flare_tff_example_server1_a_rrdatas" {
  default     = []
  description = "Value of the A record for server1"
  type        = list(string)
}
