...
locals {
  ...
  namespace_configuration_template_files = flatten([for _, file in flatten(fileset(local.federated_learning_nvidia_flare_tff_example_templates_directory_path, "**")) : "${local.federated_learning_nvidia_flare_tff_example_templates_directory_path}/${file}"])
