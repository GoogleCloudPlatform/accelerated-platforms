...
if [[ ${deploy_on_gpu} == "true" ]]; then
  echo "P/D disaggregation is currently only supported on TPU in this repository"
  echo "Set 'llmd_accelerator_type' to a supported TPU accelerator in _shared_config/llmd-shared.auto.tfvars"
  exit 0
...
echo "llm-d pd-disaggregation deploy total runtime: $(date -d@${total_runtime_value} -u +%H:%M:%S)"
