...
resource "local_file" "gradio" {
  ...
  filename        = "${local.namespace_directory}/${local.ira_online_gpu_kubernetes_namespace_name}/frontend/gradio.yaml"
}
...
module "kubectl_apply_gradio" {
  ...
  manifest                    = "${local.namespace_directory}/${local.ira_online_gpu_kubernetes_namespace_name}/frontend/gradio.yaml"
