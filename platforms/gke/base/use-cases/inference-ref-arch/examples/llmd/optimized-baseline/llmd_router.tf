...
data "http" "llmd_ob_router_base_values" {
  url = local.llmd_ob_router_base_helm_values
}

data "http" "llmd_ob_router_guide_values" {
  url = local.llmd_ob_router_guide_helm_values
}
...
data "helm_template" "llmd_ob_router" {
  ...
  chart      = var.llmd_ob_router_chart
  version    = var.llmd_ob_router_chart_version

  values = [
    data.http.llmd_ob_router_base_values.response_body,
    data.http.llmd_ob_router_guide_values.response_body,
    yamlencode({
      provider = {
        name = var.llmd_ob_gateway_provider_name
      }
      httpRoute = {
        create               = true
  ...
  skip_tests = var.llmd_ob_skip_router_render_tests
  validate   = var.llmd_ob_validate_router_manifests
}
...
module "kubectl_apply_llmd_ob_router_manifests" {
