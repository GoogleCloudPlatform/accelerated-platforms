...
resource "local_file" "llmd_pd_gateway_kustomization" {
  content = templatefile(
    "${path.module}/../_shared_config/templates/gateway/kustomization.yaml.tftpl",
    {
      remote_manifest = local.llmd_pd_gateway_remote_manifest
    }
  )
  filename = "${path.module}/.terraform/manifests/gateway-kustomize/kustomization.yaml"
}

resource "local_file" "llmd_pd_gcp_backend_policy_override" {
  content = templatefile(
...
module "kubectl_apply_llmd_pd_gateway_manifests" {
  depends_on = [
    local_file.llmd_pd_gateway_kustomization,
    local_file.llmd_pd_gcp_backend_policy_override,
  ]

  source                      = "../../../../../modules/kubectl_apply"
  apply_server_side           = true
  kubeconfig_file             = data.local_file.kubeconfig.filename
  manifest                    = dirname(local_file.llmd_pd_gateway_kustomization.filename)
