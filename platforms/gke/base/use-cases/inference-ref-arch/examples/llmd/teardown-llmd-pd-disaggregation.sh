...
if [[ ${deploy_on_gpu} == "true" ]]; then
  ...
  echo "online_tpu terraservice will be destroyed"
else
...
declare -a use_case_terraservices=(
  "../examples/llmd/pd-disaggregation"
...
echo "llm-d pd-disaggregation teardown total runtime: $(date -d@${total_runtime_value} -u +%H:%M:%S)"
