...
--config="container-images/cpu/async-load-generator/cloudbuild.yaml" \
...
EOT
  ...
  triggers_replace = {
    cloudbuild_yaml_hash = filebase64sha256("${local.acp_root}/container-images/cpu/async-load-generator/cloudbuild.yaml")
    dockerfile_hash      = filebase64sha256("${local.acp_root}/container-images/cpu/async-load-generator/Dockerfile")
    source_hash          = sha256(join("", [for file in fileset("${local.acp_root}/container-images/cpu/async-load-generator/src", "**") : filesha256("${local.acp_root}/container-images/cpu/async-load-generator/src/${file}")]))
  }
}
