...
locals {
  image_destination = local.ira_offline_batch_cpu_worker_image_url
}
...
--config="container-images/cpu/offline-batch-worker/cloudbuild.yaml" \
...
EOT
  ...
  triggers_replace = {
    cloudbuild_yaml_hash = filebase64sha256("${local.acp_root}/container-images/cpu/offline-batch-worker/cloudbuild.yaml")
    dockerfile_hash      = filebase64sha256("${local.acp_root}/container-images/cpu/offline-batch-worker/Dockerfile")
    source_hash          = sha256(join("", [for file in fileset("${local.acp_root}/container-images/cpu/offline-batch-worker/src", "**") : filesha256("${local.acp_root}/container-images/cpu/offline-batch-worker/src/${file}")]))
  }
}
