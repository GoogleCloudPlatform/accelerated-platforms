...
locals {
  image_destination = local.ira_online_tpu_max_diffusion_sdxl_image_url
}
...
--config="container-images/tpu/max-diffusion-sdxl/cloudbuild.yaml" \
...
EOT
  ...
  triggers_replace = {
    cloudbuild_yaml_hash = filebase64sha256("${local.acp_root}/container-images/tpu/max-diffusion-sdxl/cloudbuild.yaml")
    dockerfile_hash      = filebase64sha256("${local.acp_root}/container-images/tpu/max-diffusion-sdxl/Dockerfile")
    source_hash          = sha256(join("", [for file in fileset("${local.acp_root}/container-images/tpu/max-diffusion-sdxl/src", "**") : filesha256("${local.acp_root}/container-images/tpu/max-diffusion-sdxl/src/${file}")]))
  }
}
