...
locals {
  ...
  ira_online_tpu_ksa_member   = "${local.cluster_wi_principal_prefix}/ns/${local.ira_online_tpu_kubernetes_namespace_name}/sa/${local.ira_online_tpu_kubernetes_service_account_name}"
}

resource "google_storage_bucket_iam_member" "hub_models_ira_online_tpu_ksa" {
  bucket = data.google_storage_bucket.hub_models.name
  member = local.ira_online_tpu_ksa_member
