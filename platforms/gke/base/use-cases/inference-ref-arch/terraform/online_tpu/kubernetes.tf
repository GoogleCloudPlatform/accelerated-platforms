...
locals {
  ...
  ira_online_tpu_manifests_directory = "${local.namespaces_directory}/${local.ira_online_tpu_kubernetes_namespace_name}"
...
resource "local_file" "namespace_yaml" {
  content = templatefile(
    ...
    {
      name = local.ira_online_tpu_kubernetes_namespace_name
    }
  )
  filename = "${local.namespaces_directory}/namespace-${local.ira_online_tpu_kubernetes_namespace_name}.yaml"
}
...
resource "local_file" "serviceaccount_yaml" {
  content = templatefile(
    ...
    {
      name      = local.ira_online_tpu_kubernetes_service_account_name
      namespace = local.ira_online_tpu_kubernetes_namespace_name
    }
  )
  filename = "${local.ira_online_tpu_manifests_directory}/serviceaccount-${local.ira_online_tpu_kubernetes_service_account_name}.yaml"
}
...
module "kubectl_apply_service_account" {
  ...
  apply_server_side           = true
  kubeconfig_file             = data.local_file.kubeconfig.filename
  manifest                    = "${local.ira_online_tpu_manifests_directory}/serviceaccount-${local.ira_online_tpu_kubernetes_service_account_name}.yaml"
  manifest_includes_namespace = true
}
