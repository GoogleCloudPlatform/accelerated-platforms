...
data "external" "backend_service" {
  ...
  # TODO: Figure out how to wait for results instead of failing on a race condition.
  program = ["bash", "-c", "gcloud compute backend-services list --project=${local.cluster_project_id} --filter=\"iap.oauth2ClientId=${google_iap_client.workflow_api.client_id}\" --format=\"json(name)\" | jq .[0]"]
}
