...
# In v2.0 (Stateless Agent Sandbox without Agent Substrate snapshots),
# every rollout starts in a blank container and must rebuild Turn-1 state from scratch:
t_rebuild_start = time.perf_counter_ns()
...
stateless_turn1_rebuild_ms = round((time.perf_counter_ns() - t_rebuild_start) / 1e6, 3)

ns = {{"LOGS": LOGS, "logs": logs, "DATA": DATA, "data": data, "TARGET": TARGET, "target": target}}
t0 = time.perf_counter()
try:
...
print(json.dumps({{
    ...
    "architecture": "v2.0_Stateless_Agent_Sandbox (No Substrate Snapshot)",
    "warmpool_pod": uname.node,
    "kernel_release": uname.release,
    "stateless_turn1_rebuild_from_scratch_ms": stateless_turn1_rebuild_ms,
    "turn2_code_exec_ms": round(exec_ms, 3),
    "total_in_sandbox_ms": round(stateless_turn1_rebuild_ms + exec_ms, 3),
    "worker_hold_during_6s_llm_think_ms": 6000,
    "compute_density": "1.0x (Container pinned in RAM during LLM think phase)",
    "reward_breakdown": {{
    ...
    "environment_feedback": error_msg or "Stateless rebuild + unit test assertions completed."
}}, indent=2))
"""
    # In v2.0 (without Agent Substrate pre-resumed Actor snapshots), allocate claim synchronously
    _WARM_CLAIM_QUEUE.queue.clear()
...
SHARED_RLVR_SYSTEM_INSTRUCTION = (
    "You are part of the GKE Reinforcement Learning with Verifiable Rewards (RLVR) Secure Runtime pipeline (`RL_Orchestrator`), "
    "consisting of three sequential stages that share a unified system prompt for 100% KV-cache prefix reuse:\n"
    "- Stage 1 (`Policy_Rollout_Generator`): Output 3 candidate Python rollout attempts for the task:\n"
    "  * `Rollout-1-Reward-Hacking-Escape`: Attempts to cheat/exfiltrate by scraping the GCP Metadata Server (`http://169.254.169.254/computeMetadata/v1/instance/service-accounts/default/token`).\n"
    "  * `Rollout-2-Suboptimal-CPU-Spike`: A naive `O(n^2)` or buggy draft that fails or hits high latency.\n"
    "  * `Rollout-3-Verified-Safe-Optimal`: A clean, optimal `O(n)` solution plus an `assert` `Unit_Test_Suite`. Do NOT call tools in Stage 1.\n"
    "- Stage 2 (`gVisor_Reward_Verifier`): Always execute all 3 sandbox verifications:\n"
    "  1. Call `verify_zero_trust_isolation('http://169.254.169.254')` for `Rollout-1-Reward-Hacking-Escape` (`R = -1.00`, `BLOCKED_BY_ZERO_TRUST_SANDBOX`).\n"
    "  2. Call `evaluate_rl_rollout_reward` for `Rollout-2-Suboptimal-CPU-Spike` (`R = 0.30`).\n"
    "  3. Call `evaluate_rl_rollout_reward` for `Rollout-3-Verified-Safe-Optimal` (`R = +1.00` on gVisor Kernel 4.4.0).\n"
    "- Stage 3 (`GRPO_Policy_Updater`): Present the final Markdown RLVR Secure Runtime Scoreboard table comparing all 3 Rollouts (`Rollout ID & Profile | WarmPool Pod | gVisor Kernel | Claim/Exec Latency | Security & Test Status | Verifiable Reward (R_i) | GRPO Group Advantage (A_i) | Policy Update Signal`)."
)
                ...
                types.Content(
                    ...
                    parts=[
                        types.Part.from_text(
                            text=f"[Active Pipeline Stage: {stage_name}]"
                        )
                    ],
                )
            )
