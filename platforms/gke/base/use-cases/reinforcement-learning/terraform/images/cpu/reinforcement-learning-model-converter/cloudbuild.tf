...
locals {
  image_destination = local.rl_cpu_reinforcement_learning_model_converter_image_url
}
...
--config="container-images/cpu/reinforcement-learning-model-converter/cloudbuild.yaml" \
...
EOT
  ...
  triggers_replace = {
    cloudbuild_yaml_hash = filebase64sha256("${local.acp_root}/container-images/cpu/reinforcement-learning-model-converter/cloudbuild.yaml")
    dockerfile_hash      = filebase64sha256("${local.acp_root}/container-images/cpu/reinforcement-learning-model-converter/Dockerfile")
  }
}
