...
locals {
  ...
  endpoints = {
    gradio = {
      host         = "gradio.${local.mft_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 8080
      service_name = "gradio-svc"
    },
    locust = {
      host         = "locust.${local.mft_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 8089
      service_name = "locust-master-web-svc"
    },
    mlflow-tracking = {
      host         = "mlflow-tracking.${local.mft_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 5000
      service_name = "mlflow-tracking-svc"
    },
    ray-dashboard = {
      host         = "ray-dashboard.${local.mft_namespace}.${local.unique_identifier_prefix}.${local.endpoints_hostname_suffix}"
      port         = 8265
