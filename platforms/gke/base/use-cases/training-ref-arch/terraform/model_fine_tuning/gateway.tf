...
locals {
  gateway_manifests_directory = "${local.manifests_directory}/namespace/${local.mft_namespace}/gateway"
...
resource "google_compute_managed_ssl_certificate" "external_gateway" {
  ...
  name    = "${local.unique_identifier_prefix}-${local.mft_namespace}-external-gateway"
...
resource "google_compute_global_address" "external_gateway_https" {
  name    = "${local.unique_identifier_prefix}-${local.mft_namespace}-external-gateway-https"
  project = data.google_project.cluster.project_id
}
