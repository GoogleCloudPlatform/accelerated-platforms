...
locals {
  image_destination             = local.mft_data_processing_image_url
  image_directory               = "container-images/cpu/mft-data-processing/ray"
  source_folder                 = "${local.acp_root}/container-images/cpu/mft-data-processing/ray/src"
  source_data_processing_folder = "${local.acp_root}/modules/python/src/datapreprocessing"
}
...
EOT
  ...
  triggers_replace = {
    ...
    source_data_processing_hash = sha256(join("", [for file in fileset("${local.source_data_processing_folder}", "**") : filesha256("${local.source_data_processing_folder}/${file}")]))
  }
}
