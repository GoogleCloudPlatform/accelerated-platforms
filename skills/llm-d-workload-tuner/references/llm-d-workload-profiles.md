...
| **`code_coder`** / **`code-gen-bench`**          | Code generation assistant simulation.         | Large input context (e.g. codebase context files), long output code blocks. | **Tensor Parallelism Scaling:** Scale Tensor Parallel size to reserve VRAM space for large output generation memory.    |
