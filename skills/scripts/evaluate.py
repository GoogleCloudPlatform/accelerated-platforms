...
if "get" in args and "customcomputeclasses" in args:
    acc = os.environ.get("ACCELERATOR_TYPE", "nvidia-l4")
    if acc == "nvidia-k80":
...
print('{"object":"list","data":[{"id":"google/gemma-3-1b-it","object":"model"}]}')
""")

    # Mock llm-d
    write_mock_script("llm-d", """
import sys
import os
import json
...
def clean_outputs():
    ...
    for file in ["results.json", "report_v0.2.json", "dcgm_metrics.json", "output.csv"]:
...
def check_assertion(assertion_str, stdout, stderr):
    ...
    if "platform_name is updated" in assertion_lower:
        ...
        if len(matches) < 2:
            return False, "Could not extract model/accelerator from assertion description"
        ...
        if model_check in stdout and acc_check in stdout:
            return True, f"Stdout verified model/accelerator inputs: {model_check}, {acc_check}"
        else:
            return False, f"Stdout did not contain model ({expected_model}) or accelerator ({expected_acc})"
        ...
        if not overlay_path:
            ...
            if match:
                overlay_path = "platforms/gke/base/use-cases/inference-ref-arch/kubernetes-manifests/online-inference-gpu/vllm/" + match.group(1)
        if not overlay_path:
    ...
    elif "returns a warning or error about nvidia-k80" in assertion_lower or "unsupported or missing custom compute class" in assertion_lower:
        k80_err = "Warning: No custom compute class explicitly matching nvidia-k80 found."
        ...
        return True, "Verified run_benchmark.sh invocation arguments structured correctly"

    elif "benchmark endpoint validation curl command was executed" in assertion_lower or "curl command to check v1/models was executed" in assertion_lower:
        ...
        if os.path.exists(MOCK_LOG_FILE):
            ...
            if "gcloud storage cp" in calls and "gs://llm-d-benchmark/" in calls:
                return True, "Verified gcloud storage cp to gs://llm-d-benchmark/ was executed"
        ...
        if os.path.exists(MOCK_LOG_FILE):
            with open(MOCK_LOG_FILE, "r") as f:
                calls = f.read()
            if expected_cluster and expected_cluster in calls and expected_zone and expected_zone in calls:
                return True, f"Found CLUSTER_NAME='{expected_cluster}' and ZONE='{expected_zone}' usage in commands"
        return False, f"CLUSTER_NAME/ZONE usage not verified (cluster: {expected_cluster}, zone: {expected_zone})"
...
def run_test_case(skill_name, case, mock_mode):
    ...
    if "gemma-l4-cluster" in case["prompt"]:
        cluster_name = "gemma-l4-cluster"
        env["HF_MODEL_ID"] = "google/gemma-3-1b-it"
        env["ACCELERATOR_TYPE"] = "nvidia-l4"
    elif "qwen-h100-cluster" in case["prompt"]:
        cluster_name = "qwen-h100-cluster"
        env["HF_MODEL_ID"] = "Qwen/Qwen3-Coder-32B-Instruct"
    ...
    try:
        ...
        if skill_name == "llm-d-deploy-stack":
            ...
            res = subprocess.run(
                ["bash", "skills/scripts/run_benchmark.sh", "skills/llm-d-code-gen-bench/config.json", "http://vllm-eval:8000"],
                capture_output=True, text=True, env=env
            )
            stdout, stderr = res.stdout, res.stderr
        elif skill_name == "llm-d-interactive-chat":
            res = subprocess.run(
                ["bash", "skills/scripts/run_benchmark.sh", "skills/llm-d-interactive-chat/config.json", "http://vllm-eval:8000"],
                capture_output=True, text=True, env=env
            )
            stdout, stderr = res.stdout, res.stderr
    ...
    print(f"Status: {'SUCCESS' if all_passed else 'FAILURE'} (Duration: {duration_ms}ms)")
    for res in results:
...
def main():
    ...
    parser.add_argument("--eval-file", default=None, help="Path to specific evals JSON file. If omitted, autodiscovers skills/*/evals/evals.json")
    parser.add_argument("--mock", action="store_true", help="Run with mock CLI environment")
    ...
    if args.eval_file:
        ...
        if isinstance(data, list):
            ...
            parent_dir = os.path.basename(os.path.dirname(os.path.dirname(args.eval_file)))
            ...
            print("Error: Unknown eval file format (must be a list of cases or a dict with 'skills' key)")
            sys.exit(1)
    else:
                        ...
                        try:
                            with open(eval_path, "r") as f:
                                ...
                                if isinstance(cases, list):
                                    ...
                                    print(f"Warning: {eval_path} does not contain a JSON list.")
                        except Exception as e:
    ...
    summary = {
        ...
        "success_rate": float(total_passed) / total_scenarios if total_scenarios > 0 else 0.0,
        "results": report
    }
