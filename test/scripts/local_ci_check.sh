...
check_kustomize() {
  ...
  FAILED_BUILD=0
  # Exclude only directories literally named "base". Filtering on the full path
  # with "*/base/*" would match this repository's own platforms/gke/base/
  # prefix and therefore exclude every manifest.
  MANIFEST_DIRS="$(find "${ACP_PLATFORM_BASE_DIR}/use-cases/inference-ref-arch/kubernetes-manifests" -name "kustomization.yaml" -exec dirname {} \; | grep -v '/base$' | sort)"

  if [ -z "${MANIFEST_DIRS}" ]; then
    error "No Kustomize manifest directories were found. The search path or filter is wrong."
    return 1
  fi
  ...
  for dir in ${MANIFEST_DIRS}; do
    CHECKED_BUILD=$((CHECKED_BUILD + 1))
    if BUILD_ERR="$(kubectl kustomize "${dir}" 2>&1 >/dev/null)"; then
      success "Kustomize build OK: $(basename "${dir}")"
      continue
    fi

    # A build can fail because a file is generated by a configure_*.sh script
    # and is not committed. That is expected before the script has been run.
    # It is only expected, though, if a template exists to generate the file;
    # a file that is simply missing is a real defect.
    MISSING_PATH="$(echo "${BUILD_ERR}" | grep -o "lstat [^:]*: no such file" | head -1 | sed -e 's/^lstat //' -e 's/: no such file$//')"
    GENERATED=false
    if [ -n "${MISSING_PATH}" ]; then
      MISSING_DIR="$(dirname "${MISSING_PATH}")"
      MISSING_FILE="$(basename "${MISSING_PATH}")"
      TEMPLATE="${MISSING_DIR}/templates/${MISSING_FILE%.*}.tpl.${MISSING_FILE##*.}"
      [ -f "${TEMPLATE}" ] && GENERATED=true
    fi

    if [ "${GENERATED}" = true ]; then
      warn "Kustomize build SKIPPED (${MISSING_FILE} not generated yet): $(basename "${dir}")"
      SKIPPED_BUILD=$((SKIPPED_BUILD + 1))
    else
      error "Kustomize build FAILED: ${dir}"
      echo "${BUILD_ERR}" | head -3
      FAILED_BUILD=$((FAILED_BUILD + 1))
    fi
  done

  if [ ${FAILED_BUILD} -eq 0 ]; then
    success "Kustomize: $((CHECKED_BUILD - SKIPPED_BUILD)) validated, ${SKIPPED_BUILD} skipped, 0 failed."
  else
    error "${FAILED_BUILD} of ${CHECKED_BUILD} Kustomize manifest directory(s) failed build check."
    return 1
  fi
}
