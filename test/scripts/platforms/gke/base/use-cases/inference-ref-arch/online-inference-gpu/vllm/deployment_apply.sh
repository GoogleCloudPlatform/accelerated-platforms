...
echo "Deploying model(s) on ${ACCELERATOR_TYPE}"
echo
...
for model in "${hf_gpu_vllm_models[@]}"; do
  ...
  if [[ -d "${ACP_REPO_DIR}/platforms/gke/base/use-cases/inference-ref-arch/kubernetes-manifests/online-inference-gpu/vllm/${ACCELERATOR_TYPE}-${HF_MODEL_NAME}" ]]; then
    echo "Deploying '${HF_MODEL_ID}' model resources..."
    echo "--------------------------------------------------------------------------------------------"
