...
Once a model has completed fine-tuning and is deployed on GKE , its ready to run batch Inference pipeline. In this example batch inference pipeline, we would first send prompts to the hosted fine-tuned model and then validate the results based on ground truth.
...
You can review predictions result in file named predictions.txt .Sample file has been added to the repository. The job will take approx 45 mins to execute.
