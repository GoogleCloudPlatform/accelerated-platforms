...
- Replace the respective variables required for the job
  ...
  sed \
    ...
    -i -e "s|V_FLAT_BUCKET|${MLP_STORAGE_BENCHMARK_FLAT_BUCKET}|" \
    -i -e "s|V_HIERARCHICAL_BUCKET|${MLP_STORAGE_BENCHMARK_HIERARCHICAL_BUCKET}|" \
    -i -e "s|V_MODEL_REPO|${MODEL_REPO}|" \
    manifests/transfer-llama-to-gcs-${ACCELERATOR}-dws.yaml
  ```
...
- Check the status of the provisioning request, once the `PROVISIONED` column
  ...
  ```sh
  watch -n 5 kubectl --namespace ${MLP_KUBERNETES_NAMESPACE} get provisioningrequest a100-gcs-transfer-job
  ```
