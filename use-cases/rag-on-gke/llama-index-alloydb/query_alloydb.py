...
class AlloyDBNaiveRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        if isinstance(table, str):
            self._db_meta_data.reflect(bind=self._db_engine,
                                       schema=table_schema)
            self._db_table = (self._db_meta_data.
                              tables[f"{table_schema}.{table}"])
        else:
            self._db_table = table
        self._table_embedding_column = (self._db_table.
                                        c[embedding_column])
        self._text_column = text_column
        self._metadata_columns = metadata_columns
        self._id_column = id_column
        self._table_all_columns = [self._db_table.c[x] for x in
                             set([text_column] + metadata_columns +
                                 [id_column])]
    ...
    def _row_to_nodewithscore(self, row):
        ...
        node = TextNode(text=getattr(row, self._text_column),
                        metadata=metadata,
    ...
    def _as_sql(self,
                query_bundle: QueryBundle) -> Select:
        ...
        stmt = (sqlalchemy.select(*columns).
    ...
    def as_subquery(self,
                    query_bundle_or_str:
                    Union[str, QueryBundle]) -> Subquery:
        ...
        return (sqlalchemy.
                select(sqlalchemy.
                       func.
                       string_agg(subq.c[self._text_column], "\n").
                       label("context")).
                select_from(subq).
                subquery())
    ...
    def _retrieve(self,
        ...
        return [self._row_to_nodewithscore(r)
                for r in results]
...
class AlloyDBNaiveQueryEngine(CustomQueryEngine):
    ...
    def _synth_sql(self, query_str: str) -> Select:
        ...
        prompt = self.qa_prompt.format(query_str=query_str,
                                      context_str="%1$s")
        stmt = (sqlalchemy.
                select(self._inference_func(sqlalchemy.
                                           func.
                                           format(prompt,
                                                  subq.c.context))).
                select_from(subq))
...
if __name__ == "__main__":
    ...
    test_retriever = AlloyDBNaiveRetriever(url=dburl,
